...
def AgentMotion(pob,ob,movethreshold):
    diff=np.sum(np.absolute(pob-ob))
...
class SamplingReplayBuffer(object):
    ...
    def _encode_sample(self, idxes):
        ...
        idxes_next=[idx+1 for idx in idxes]
        for i in range(len(idxes)):
            if self.done[idxes[i] % self.size]:
                idxes_next[i] = idxes[i]
        obs_batch      = np.concatenate([self._encode_observation(idx)[None] for idx in idxes], 0)
        act_batch      = self.action[idxes]
        rew_batch      = self.reward[idxes]
        envid_batch    = self.env_id[idxes]
        pos_batch      = self.pos[idxes]
        pos_batch_next = self.pos[idxes_next]
        next_obs_batch = np.concatenate([self._encode_observation(idx)[None] for idx in idxes_next], 0)
        done_mask      = np.array([1.0 if self.done[idx] else 0.0 for idx in idxes], dtype=np.float32)

        return obs_batch, act_batch, rew_batch,next_obs_batch, done_mask,envid_batch,pos_batch,pos_batch_next
    ...
    def sample(self, batch_size):
        ...
        while not is_ok:
            ...
            if random.random>0.5:
                start_idx=self.moveset[random.randint(0, len(self.moveset) - 1)]
            else:
                start_idx=self.nomoveset[random.randint(0, len(self.nomoveset) - 1)]
            end_idx = start_idx+batch_size
            if end_idx>self.num_in_buffer:
                is_ok = False
            ...
            for idx in range(start_idx, end_idx - 1):
                if self.done[idx % self.size]:
                    is_ok=False
                    break
        idxes=range(start_idx,end_idx)
        '''
        idxes = sample_n_unique(lambda: self.moveset[random.randint(0, len(self.moveset) - 1)],batch_size-batch_size//4)
        idxes.extend(sample_n_unique(lambda: self.nomoveset[random.randint(0, len(self.nomoveset) - 1)], batch_size//4))
    ...
    def store_frame(self, frame):
        ...
        if self.obs is None:
            ...
            self.env_id   = np.empty([self.size],                     dtype=np.int32)
            self.done     = np.empty([self.size],                     dtype=np.bool)
            self.pos     = np.empty([self.size,2],                    dtype=np.float32)
            self.moveset  = []
            self.nomoveset= []
    ...
    def store_effect(self, idx, action, reward, done, env_id,pos):
        ...
        self.done[idx]   = done
        self.env_id[idx] = env_id
        self.pos[idx] = pos

    def store_dx(self,idx,dx):

        if dx==1:
            self.moveset.append(idx)
        else:
            self.nomoveset.append(idx)
...
def x_process_frame(frame):
    #frame = frame[15:150, 15:150,:]
    ...
    resized_screen = cv2.resize(frame, (80, 80),  interpolation=cv2.INTER_LINEAR)
    #x_t = np.reshape(resized_screen, [84, 84, 3])
    #return x_t.astype(np.uint8)
    #print ("frame_shape", np.shape(resized_screen))
    #print ("type", resized_screen.dtype)
    #imsave('test.jpg',resized_screen)
...
class ProcessFrame(gym.Wrapper):
    ...
    def _step(self, action):
        ...
        return x_process_frame(obs), reward, done, info

    def _reset(self):
        return x_process_frame(self.env.reset())
